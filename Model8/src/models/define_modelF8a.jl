struct ModelF8a <: AbstractFlexiModel
...
function make_ModelF8a(; flexi_dofs=20)
    ...
    params_repr_ig = ComponentArray(
        p_classical=represent_on_type(p_classical_derepresented_ig, ModelF8a),
...
function make_rhs(i_func, model::ModelF8a)

    # DI-IFFL with flexi1 in m-PP and flexi2 in m-DI

    function rhs(du, u, p_all_derepresented, t)
        a, m, w, c = u
        ta, tm, tw, tc, beta_ma, nwm, nam = p_all_derepresented.p_classical
...
function represent_on_type(p_derepresented, model_by_type::Type{ModelF8a})
...
function derepresent(p_repr, model::ModelF8a)
    return ComponentArray(
