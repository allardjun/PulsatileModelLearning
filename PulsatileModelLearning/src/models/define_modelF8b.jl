struct ModelF8b <: AbstractFlexiModel
...
function make_ModelF8b(; flexi_dofs=20)
    ...
    p_derepresented_lowerbounds = ComponentArray(
        ...
        tau_m=1.0f0,
        tau_w=1.0f0,
        tau_c= 1440*10.0, # prevent numerical/biological underflow regime. 
        beta_mw = 1.0*1e-3,
        # beta_ma = 1.0*1e-3,
        nwm=0.5,
        nam=0.5,
    ...
    params_repr_ig = ComponentArray(
        p_classical=represent_on_type(p_classical_derepresented_ig, ModelF8b),
...
function make_rhs(i_func, model::ModelF8b)

    # DI-IFFL with flexi1 in m-PP and flexi2 in m-DI

    function rhs(du, u, p_all_derepresented, t)
        a, m, w, c = u
        ta, tm, tw, tc, beta_mw, nwm, nam = p_all_derepresented.p_classical
...
function represent_on_type(p_derepresented, model_by_type::Type{ModelF8b})
...
function derepresent(p_repr, model::ModelF8b)
    return ComponentArray(
