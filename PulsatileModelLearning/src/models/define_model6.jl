...
struct Model6 <: AbstractClassicalModel
...
function make_Model6()
    ...
    params_repr_ig = ComponentArray( p_classical=represent_on_type(p_classical_derepresented_ig, Model6))
...
function make_rhs(i_func, model::Model6)

    # linear architecture with decay-inhibition

    function rhs(du, u, p_all_derepresented, t)
        a, m, w, c = u
        ta, tm, tc, beta_ma, nam = p_all_derepresented.p_classical

        du[1] = (1 / ta) * (i_func(t) - a)
        du[2] = (1 / tm) * (a - 1 / (1 + (abs.(beta_ma * a))^nam) * m)
        du[3] = 0
        du[4] = (m - c / tc)
...
function represent_on_type(p_derepresented, model_by_type::Type{Model6})
...
function derepresent(p_repr, model::Model6)
    return ComponentArray(
