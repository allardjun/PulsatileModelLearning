struct ModelF7 <: AbstractFlexiModel
...
function make_ModelF7(; flexi_dofs=20)
    ...
    params_repr_ig = ComponentArray(
        p_classical=represent_on_type(p_classical_derepresented_ig, ModelF7),
...
function make_rhs(i_func, model::ModelF7)

    # Incoherent feedforward with flexi function

    function rhs(du, u, p_all_derepresented, t)
        a, m, w, c = u
        ta, tm, tw, tc  = p_all_derepresented.p_classical

        du[1] = (1 / ta) * (i_func(t) - a)
        # Add flexi function to the incoherent feedforward term
        du[2] = (1 / tm) * (
...
function represent_on_type(p_derepresented, model_by_type::Type{ModelF7})
...
function derepresent(p_repr, model::ModelF7)
    return ComponentArray(
